    ...
    public function accion(Request $request)
{
    ...
    if ($accion == 'comprar') {
        // Verificar si ya posee el videojuego
        $existe = DB::table('posesiones')->where('videojuego_id', $videojuego)->where('user_id', $user)->exists();
    ...
    if ($accion == 'vender') {
        // Verificar si posee el videojuego
        $existe = DB::table('posesiones')->where('videojuego_id', $videojuego)->where('user_id', $user)->exists();

        if ($existe) {
            // Eliminar el videojuego de la tabla de posesiones
            DB::table('posesiones')
                ->where('videojuego_id', $videojuego)
                ->where('user_id', $user)
                ->delete();
    ...
    // Si no es 'comprar' ni 'vender', redirigir a la página de videojuegos
    return redirect()->route('videojuegos.index');
}
